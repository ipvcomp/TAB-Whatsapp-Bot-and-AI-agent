...
async def _send_text(to: str, body: str, phone_number_id: Optional[str]):
    await send_text_message(
        to=to, body=body, phone_number_id=phone_number_id, source="bp_link_flow"
    )
    await send_text_message(
        to=to, body=_UTILITY, phone_number_id=phone_number_id, source="bp_link_flow"
    )
...
async def _send_list(
    ...
    await send_whatsapp_payload(
        whatsapp_payload=payload, phone_number_id=phone_number_id, source="bp_link_flow"
    )
    await send_text_message(
        to=to, body=_UTILITY, phone_number_id=phone_number_id, source="bp_link_flow"
    )
...
async def _show_policy_list(
    wa_id: str, session: dict, flow: dict, action: str, phone_number_id: Optional[str]
):
    flow["step"] = "bp_policy"
    await save_session(session)
    body = (
...
async def _ask_upload(
    wa_id: str, session: dict, flow: dict, pol: dict, phone_number_id: Optional[str]
):
...
async def _show_upload_confirmed(
    wa_id: str, session: dict, flow: dict, phone_number_id: Optional[str]
):
...
async def _show_payout_initiated(
    wa_id: str, session: dict, flow: dict, phone_number_id: Optional[str]
):
        ...
        [
            {
                "title": "Options",
                "rows": [
                    {"id": "bp_upload_me", "title": "📤 Upload for me"},
                    {"id": "bp_link_pol", "title": "🔗 Link to active policy"},
                ],
            }
        ],
        phone_number_id,
...
async def handle_bp_link_flow(
    ...
    if message.type == "image" and hasattr(message, "image") and message.image:
        ...
        media = {
            ...
            or (img.get("id") if isinstance(img, dict) else None),
            "filename": f"boarding_pass_{data.get('bp_sel_flight', '')}.jpg",
        }
    elif (
        message.type == "document" and hasattr(message, "document") and message.document
    ):
        ...
        media = {
            ...
            or (doc.get("filename") if isinstance(doc, dict) else None)
    ...
    if step == "bp_choose":
        if reply_id == "bp_upload_me":
            ...
            await _show_policy_list(
                sender_wa_id, session, flow, "upload", phone_number_id
            )
        elif reply_id == "bp_link_pol":
            data["bp_action"] = "link"
            await _show_policy_list(
                sender_wa_id, session, flow, "link", phone_number_id
            )
        else:
        ...
        if reply_id and reply_id.startswith("bpp_"):
            try:
                ...
                if 0 <= idx < len(DEMO_POLICIES):
                    ...
                    await save_session(session)
                    if action == "link":
                        await _show_link_confirm(
                            sender_wa_id, session, flow, phone_number_id
                        )
                    else:
                        await _ask_upload(
                            sender_wa_id, session, flow, pol, phone_number_id
                        )
                else:
                    await _show_policy_list(
                        sender_wa_id, session, flow, action, phone_number_id
                    )
