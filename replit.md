...
- **Policy Creation Flow:** A guided, multi-step process for users to purchase insurance policies. MSISDN auto-derived from the user's WhatsApp number (confirmed at flow start). Flow: (1) Greetings → (2) Service offering → (3) MSISDN confirm → (4) Departure Airport (search by first 3 chars of name/code, uses airport API `country_iso2` to permanently set user's country) → (5) Product selection (fetched by airport country) → (6) Product confirmation → (7) Itinerary intro → (8) Dep date → (9) Dep time → (10) Arrival airport → (11) Arr date → (12) Arr time → (13) Booking ref → (14) Flight no → (15) First name → (16) Last name → (17) Email → (18) KYC (ID type + number) → (19) Details confirmation → (20) Payment intro + method → (21) Account number → (22) Bank name search (3 chars) → (23) Bank selection → (24) Boarding pass choice (Upload now / Upload later) → (25) Policy summary confirmation (Yes, Submit / No, Change details) → (26) Policy submission + final summary. Itinerary fields use direct code validation (no LLM). Names use LLM extract only when user provides a sentence instead of a single name; otherwise accepted directly. Bank search fetches all banks, filters locally by user's 3-char input, then shows matching banks. Boarding pass can be uploaded later via the "Submit Boarding Pass" welcome button.
