...
- **Ipurvey APIs (for policy flow):**
    ...
    - Policies by MSISDN API: `GET https://dev-ilekun-ipv.ipurvey.com/api/tab-plc/policies/by-msisdn/{msisdn}` — used by `bp_link_flow_service` and `check_policy_flow_service` to fetch live user policies (replaces DEMO_POLICIES). Implemented in `app/services/ipurvey_api.py` with field normalization and graceful error handling (empty list on 404/errors).