...
- **Policy Creation Flow:** A guided, multi-step process for users to purchase insurance policies. Country and MSISDN are auto-derived from the user's WhatsApp number (confirmed at flow start). Flow: MSISDN confirm → Product selection → Product confirmation → Itinerary (dep date, dep time, arrival airport search, arr date, arr time, booking ref, flight no) → Personal details (first name, last name, email) → ID verification (NIN or BVN, 11-digit validated) → Details confirmation (shows all itinerary + passenger data with "Yes, Proceed" / "No, Change details" buttons) → Payment intro message → Payment method → Payout method (hardcoded Bank Transfer) → Account number (10-digit validated) → Bank selection → Departure Airport → Boarding pass upload (JPG/PNG/WebP/PDF) → Policy submission → Final summary. This flow takes precedence over LLM interactions. Itinerary fields use direct code validation (no LLM). Names use LLM extract only when user provides a sentence instead of a single name; otherwise accepted directly.
