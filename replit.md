...
- **Policy Creation Flow:** A guided, multi-step process for users to purchase insurance policies. Country and MSISDN are auto-derived from the user's WhatsApp number (confirmed at flow start). Flow: MSISDN confirm → Product selection → Personal details (name, email) → ID verification (NIN or BVN, 11-digit validated, stored as separate fields) → Payment method → Payout method (from API, currently Bank Transfer only) → Account number (10-digit validated) → Bank selection → Departure Airport → Itinerary (booking ref, flight no, carrier, departure date/time, arrival airport search, arrival date/time) → Boarding pass upload (JPG/PNG/WebP/PDF) → Policy submission → Final summary. This flow takes precedence over LLM interactions.
