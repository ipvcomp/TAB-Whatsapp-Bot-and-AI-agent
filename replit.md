...
- **Policy Creation Flow:** A guided, multi-step process for users to purchase insurance policies. MSISDN auto-derived from the user's WhatsApp number (confirmed at flow start). Flow: (1) Greetings → (2) Service offering → (3) MSISDN confirm → (4) Nationality (auto-suggested from phone number with confirm/change buttons; manual entry if undetectable; sets user's country permanently) → (5) Departure Airport (search by first 3 chars of name/code) → (6) Product selection (fetched by nationality country) → (7) Product confirmation → (8) Itinerary intro → (9) Dep date → (10) Dep time → (11) Arrival airport → (12) Arr date → (13) Arr time → (14) Booking ref → (15) Flight no → (16) First name → (17) Last name → (18) Email → (19) KYC (ID type + number) → (20) Details confirmation → (21) Payment intro + method → (22) Account number → (23) Bank name search (3 chars) → (24) Bank selection → (25) Boarding pass choice (Upload now / Upload later) → (26) Policy summary confirmation (Yes, Submit / No, Change details) → (27) Policy submission + final summary. Itinerary fields use direct code validation (no LLM). Names use LLM extract only when user provides a sentence instead of a single name; otherwise accepted directly. Bank search fetches all banks, filters locally by user's 3-char input, then shows matching banks. Boarding pass can be uploaded later via the "Submit Boarding Pass" welcome button.
